...
void static_update_cell(int i, int j, int size, std::vector<int>& grid, std::vector<int>& old_grid) {
  ...
  switch(alive){
    case(0):
      grid[ij_to_k(i, j, size)] = 0;break;
    case(1):
      grid[ij_to_k(i, j, size)] = 0;break;
    case(2):
      grid[ij_to_k(i, j, size)]=old_grid[ij_to_k(i,j,size)];break;
    case(3):
      grid[ij_to_k(i, j, size)]=1;break;
    case(4):
      grid[ij_to_k(i, j, size)]=0;break;
    case(5):
      grid[ij_to_k(i, j, size)]=0;break;
...
/*  if (alive > 3) {
    grid[ij_to_k(i, j, size)] = 0;}
  if (alive==2){
    grid[ij_to_k(i, j, size)] = old_grid[ij_to_k(i,j,size)];
  } else {
    grid[ij_to_k(i, j, size)] = 1;
  }*/
}
...
std::string num_conv(int number){
  std::stringstream ss;
...
void createSnapshot(std::vector<int>& arr, int size,int step) {
  ...
  std::ofstream snapshot("snapshot_" + num +".pbm");
